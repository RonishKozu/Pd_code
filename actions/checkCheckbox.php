    ...
    if(isset($_GET['broadcast'])){
        ...
        if ($result = $conn->query($sql)){
            if ($result->num_rows > 0){
                $query = "UPDATE `broadcast_notification` SET `notify`=1 WHERE `broadcast_id` = $broadcastId AND `user_id`=$userId;";
                mysqli_query($conn, $query);
            }else{
                $query = "INSERT INTO `broadcast_notification`(`broadcast_id`, `user_id`, `notify`) VALUES ({$broadcastId}, {$userId}, 1);";
                mysqli_query($conn, $query);
            }
