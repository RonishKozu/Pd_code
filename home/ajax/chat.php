...
if ($_POST['method'] === 'fetch') {
    $currentBroadcast = isset($_SESSION["SESSION_CURRENT_BROADCAST"]) ? $_SESSION["SESSION_CURRENT_BROADCAST"] : false;
    if(isset($_SESSION["SESSION_CURRENT_BROADCAST"])){        
        $messages = $chat->fetchMessage($currentBroadcast);
        ...
        if (!empty($messages)) {
            ...
            echo "YOU ARE THE FIRST ONE TO COMMENT";
        }
    }
}
...
if ($_POST['method'] === 'throw' && isset($_SESSION['userName'])) {
    ...
    var_dump($id);
    $chat->throwMessage((int)$id[0]['user_id'], $message, $_SESSION["SESSION_CURRENT_BROADCAST"]);
